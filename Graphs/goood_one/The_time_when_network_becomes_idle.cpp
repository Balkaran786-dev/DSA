...
public:
    int networkBecomesIdle(vector<vector<int>>& edges, vector<int>& patience) {
        ...
        int res = 0;
        for(int i = 1; i<n; i++) {
            int extraPayload = (time[i]*2 - 1)/patience[i];  //extra packets 
            ...
            //1 packet patience[i] time ke baad nikla, extra packets fir extraPayload * patience[i] time ke baad nikle, mtlab jo last packet iss time pe nikla , uske baad it just need to complete the round trip time.  so , total time till the last messgaes is received, is lastOut + rtt
